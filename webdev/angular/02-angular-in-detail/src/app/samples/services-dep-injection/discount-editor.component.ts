 ...
 * for handling the shared object. Similar situation happens in case where directives expect
 * to receive the shared object. The component whose template uses such a directive must
 ...
 * as illustrated by the DiscountEditorSharingObjectsViaDepInjectionComponent component.
 */
@Component({
  selector: "discount-editor-sharing-objects-via-input-props",
  template: `
  <div class="form-group">
      <label for="discount">Enter discount value:</label>
      <input type="number" name="discount" id="discount" class="form-control" [(ngModel)]="$any(discountService).discount">
  </div>`
})
export class DiscountEditorSharingObjectsViaInputPropsComponent {
  @Input("discounter")
 ...
 */
@Component({
  selector: "discount-editor-sharing-objects-via-dep-injection",
  template: `
    <div class="form-group">
      <label for="discount">Enter discount value:</label>
      <input type="number" name="discount" id="discount" class="form-control" [(ngModel)]="discountService.discount">
    </div>`
})
export class DiscountEditorSharingObjectsViaDepInjectionComponent {
