...
function parseEnvConfig(): EnvConfig {
  ...
  if (process.env.ENV_CONFIG_PATH == null || process.env.ENV_CONFIG_PATH === '') {
    console.log('Environment configuration file must be provided via the ENV_CONFIG_PATH environment variable.');
    process.exit(-1);
  }

  // Build full path to the environment configuration file
  const envConfigPath = path.join(__dirname, '..', '..', process.env.ENV_CONFIG_PATH);
...
class Environment {
  ...
  // get SERVER_ADDRESS(): string {
  //   if (process.env.NODE_ENV !== 'production') {
  //     if (this.envConfig.SERVER_ADDRESS == null) {
  ...
  //       console.log('Unexpected use of the SERVER_ADDRESS environment variable.');
  //       process.exit(-1);
  //     }
  //   }
